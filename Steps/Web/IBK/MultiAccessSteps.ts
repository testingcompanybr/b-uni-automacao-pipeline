...
When('clico no botão Criar usuário na tela de Multi acessos', async function (this: World) {
  const multiAccessActions = new MultiAccessActions(this.driver as WebDriver, this);
...
When('preencho o campo Login com o valor aleatório na tela de Credenciais multi acesso', async function (this: World) {
  const multiAccessActions = new MultiAccessActions(this.driver as WebDriver, this);
  await multiAccessActions.fillLoginRandom();
});

Then('verifico se o usuário com login gerado anteriormente foi criado na tela de Multi acessos', async function (this: World) {
  const multiAccessActions = new MultiAccessActions(this.driver as WebDriver, this);
  const generatedLogin = multiAccessActions.getStoredValue('generatedLogin');

  if (!generatedLogin) {
    throw new Error("O login gerado anteriormente não está disponível.");
  }
  const driver = this.driver as WebDriver;
  const locator = MultiAccessElementsMap.cardByLogin(generatedLogin);

  await driver.wait(until.elementLocated(locator), 10000);

  const elements = await driver.findElements(locator);
  expect(elements.length, `Card com login '${generatedLogin}' não foi encontrado`).to.be.greaterThan(0);
});

When('clico no card do usuário com login gerado anteriormente na tela de Multi acessos', async function (this: World) {
  const multiAccessActions = new MultiAccessActions(this.driver as WebDriver, this);
  const generatedLogin = multiAccessActions.getStoredValue('generatedLogin');
  ...
  await multiAccessActions.clickUserCardByLogin(generatedLogin);
});

Then('o card do usuário com login gerado anteriormente não deve mais estar visível na tela de Multi acessos', async function (this: World) {
  const driver = this.driver as WebDriver;
  const multiAccessActions = new MultiAccessActions(this.driver as WebDriver, this);
  const generatedLogin = multiAccessActions.getStoredValue('generatedLogin');
  ...
  await driver.wait(async () => {
    const elements = await driver.findElements(locator);
    return elements.length === 0;
  }, 10000, `O card com login '${generatedLogin}' ainda está visível na tela`);

  const elements = await driver.findElements(locator);
  expect(elements.length, `Card com login '${generatedLogin}' ainda existe`).to.equal(0);
});